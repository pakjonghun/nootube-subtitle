  ...
  useEffect(() => {
    const unlisten = listen<string>('extract-log', (event) => {
      setLogs((prev) => {
        ...
        const lastBase =
          prev.length > 0 ? prev[prev.length - 1].replace(/\d+%/, '').replace('완료!', '') : '';
    ...
    <div className="container">
      ...
      <div className="subtitle-area">
        <div className="subtitle-header">
          <span>{subtitle ? `${subtitle.split('\n').length} ${t(locale, 'lines')}` : ''}</span>
          {subtitle && (
        ...
        <div
          className={`subtitle-content ${!subtitle && status.type !== 'loading' ? 'empty' : ''} ${status.type === 'loading' ? 'log-terminal' : ''}`}
        >
          {status.type === 'loading' ? (
            <>
              {logs.map((log, i) => (
